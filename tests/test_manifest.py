"""
Tests for the Blender extension manifest.
...
class TestInstallersReadTheManifest:
    ...
    def test_installer_references_the_manifest(self, script):
        with open(os.path.join(REPO_ROOT, script), encoding="utf-8") as f:
            text = f.read()
        assert "blender_manifest.toml" in text, (
            f"{script} should read the version from blender_manifest.toml"
        )

    @pytest.mark.parametrize("script", ["install.ps1", "install.sh"])
    def test_installer_ships_the_manifest(self, script):
