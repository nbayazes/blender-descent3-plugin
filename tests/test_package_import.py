...
``descent3_importer/__init__.py`` defers every ``bpy``-touching import into
``register()`` so the package can be imported in a plain interpreter. That is
what lets the rest of this suite say ``from descent3_importer.poformat import
...`` instead of manipulating ``sys.path``. If someone adds a module-scope
``import bpy`` back to ``__init__.py``, or to ``constants``/``poformat``/
``texutil``, collection of every other test file breaks -- so these tests fail
loudly and point at the cause.
...
BPY_FREE_MODULES = [
    "descent3_importer",
    "descent3_importer.constants",
    "descent3_importer.poformat",
    "descent3_importer.texutil",
]
...
def test_bl_info_is_readable_without_bpy():
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import descent3_importer as d; print(d.bl_info['version'])",
        ],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
...
def test_operator_modules_are_not_imported_eagerly():
    ...
    """
    code = (
        "import sys, descent3_importer\n"
        "eager = [m for m in ('descent3_importer.import_pof',\n"
        "                     'descent3_importer.export_pof')\n"
        "         if m in sys.modules]\n"
