...
# Image formats we try, in priority order, when a texture name has no extension.
...
def clean_texture_dir(raw: str) -> str:
    ...
    stray whitespace, so strip both. Returns "" for an empty/blank value.
    """
    if not raw:
        return ""
...
def find_texture_image(texture_name: str, search_dirs) -> str | None:
    """Return the path of an image file matching ``texture_name``, or None.

    Each directory in ``search_dirs`` is checked in order. Within a directory an
    exact ``name + extension`` match is tried first (fast path), then a
